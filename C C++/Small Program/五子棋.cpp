...
struct rcd;//声明节点结构 结构名
typedef struct rcd* Record;//节点指针别名 // 定义变量record的指针    *Record结构
typedef struct rcd record;//节点别名 //创建一个的与rcd类型相同的变量类型   record结构
#define MAXIMUS 15 //定义棋盘大小

int p[MAXIMUS][MAXIMUS];//存储对局信息 //二维数组
char buff[MAXIMUS*2+1][MAXIMUS*4+3];//输出缓冲器
int Cx,Cy;//当前光标位置
int Now;//当前走子的玩家，1代表黑，2代表白
int wl,wp;//当前写入缓冲器的列数和行数位置 走子位置
char* showText;//在棋盘中央显示的文字信息 showText是一个char型 存放中央文字信息 头指针
int count;//回合数
int Putable;//指示当前是否可以走棋
int Exiting;//1为当场上无子并按ESC时询问是否退出程序的状态，2为非此状态
int ExiRep;//1为当回放到最后一回合并按向后时询问是否退出回放的状态，2为非此状态
Record RecBeg,RecNow;//记录的开始节点和当前节点  指针变量
...
Record newRecord()//记录节点构造函数 //返回空位置节点参数
{
...
void AddRecord()//添加记录
{
	RecNow->X=Cx;//记录坐标   //Cx给RecNow 
	RecNow->Y=Cy;
	RecNow->Next=newRecord();//创建下一个记录节点   //申请节点对象 头尾清空
	RecNow->Next->Back=RecNow;//完成双链表   //前向链
...
void CleanRecord()//清理所有记录 
{
	...
	while(RecBeg->Next!=NULL)//删除所有记录，直到越界前为止
	{
		...
		free(RecBeg);//释放当前节点
		RecBeg=n;//当前记录推至下一个记录节点 //RecBeg开始节点指针
	}
}

char* Copy(char* strDest,const char* strSrc)//修改过的字符串复制函数，会忽略末端的\0  返回一个字符型指针
//const指针指向的内容不可变 指针可以在赋值 Dest 目的 Src 源

{
	...
	return strDestCopy; //结果strSrc->strDest的首地址 返回strDestCopy首地址
}

void Initialize()//初始化一个对局函数
{
	int i,j;//循环变量
	system("title 对局中(按方向键控制光标，空格走子)，Esc撤销"); //输出字
	showText="";//重置显示信息
	count=0;//回合数归零
...
char* getCurse(int i,int j){//获得指定坐标交点位置左上格的样式，通过制表符来模拟光标的显示
	if(Putable)//可走棋时光标为粗线 //Putable 可下子 1可下 0不可
	{
		if(i==Cx){
			if(j==Cy)
				return "┏";
...
int Check()//胜负检查，即判断当前走子位置有没有造成五连珠的情况
{
	...
	for(i=1;i<5;i++)if(Cy+i<MAXIMUS&&p[Cx][Cy+i]==Now)w++;else break;//向上检查
	for(i=1;i<5;i++)if(Cy-i>0&&p[Cx][Cy-i]==Now)w++;else break;//向下检查
